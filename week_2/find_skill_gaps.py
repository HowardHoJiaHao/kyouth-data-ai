...
def get_normalized_db_skills() -> set:
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT tech_stack FROM jobs WHERE tech_stack IS NOT NULL AND tech_stack != 'FAILED'")
    rows = cursor.fetchall()
    
    # Flatten every tech skill
    raw_skills = {s.strip().lower() for row in rows for s in row[0].split(',') if s.strip()}
