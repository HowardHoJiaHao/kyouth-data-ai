...
async def chat(payload: ChatRequest):
    ...
    try:
        ...
        ai_reply = prompt_model(
            model_identifier="ds", prompt=payload.message, context=context_text
        )
        ...
        return JSONResponse(
            status_code=500,
            content={
                "response": f"[Backend Module Failure] Failed executing integrated prompt_model script: {str(e)}"
            },
        )
...
async def root():
    return {
        ...
        "status": "running",
        "version": "1.0.0",
        "endpoints": {"chat": "POST /chat", "health": "GET /health", "root": "GET /"},
    }
