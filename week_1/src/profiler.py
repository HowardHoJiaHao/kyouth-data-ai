...
def run_data_profile(db_path):
    ...
    # If there are no records, stop here to prevent indexing errors
    if total == 0:
        print("⚠️ No data available to profile.")
        conn.close()
        return

    # These will now only run if total > 0
